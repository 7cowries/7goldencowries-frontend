...
export default function Profile() {
  ...
  // Read from me.socials but also fall back to legacy top-level fields
  ...
  // Prefer TonConnect wallet; fall back to any stored value on load
  useEffect(() => {
  ...
  const handleUnlink = useCallback(async (provider) => {
    ...
    <Page>
      <div className="profile">
        ...
        <h1 className="section-title">
          🌊 <span className="yolo-gradient">Explorer Profile</span>
        </h1>
                ...
                <ul>
                  {(me.questHistory || [])
                    ...
                    .sort(
                      (a, b) =>
                        new Date(
                          b.completed_at || b.created_at || b.timestamp || 0
                        ) -
                        new Date(
                          a.completed_at || a.created_at || a.timestamp || 0
                        )
                    )
                    .map((q, i) => {
                      const when =
                        q.completed_at || q.created_at || q.timestamp;
                      const ts = when ? new Date(when) : null;
                      return (
                        <li key={q.id || i}>
                          {q.title ||
                            q.reason ||
                            `Quest #${q.quest_id ?? q.id ?? ""}`}{" "}
                          — {q.xp ?? q.delta ?? 0} XP
                          {ts ? (
