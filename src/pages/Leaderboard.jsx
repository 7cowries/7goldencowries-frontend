          ...
          {podium.map((u, i) => (
            <div key={u.wallet || i} className={`card glass podium-${i + 1} ${walletRef.current === u.wallet ? "me" : ""}`}>
              ...
              <div className="big-wallet">
                ...
                {u.twitterHandle ? (
                  <a className="muted" style={{ marginLeft: 6 }} href={`https://x.com/${u.twitterHandle}`} target="_blank" rel="noreferrer">
                    @{u.twitterHandle}
                  </a>
                ) : null}
              </div>
              ...
              return (
                <div key={u.wallet || rank} className={`row glass ${isMe ? "me" : ""}`}>
                  ...
                  <div className="wallet mono">
                    {abbreviateWallet(u.wallet)}
                    {u.twitterHandle ? (
                      <a className="muted" style={{ marginLeft: 4 }} href={`https://x.com/${u.twitterHandle}`} target="_blank" rel="noreferrer">
                        @{u.twitterHandle}
                      </a>
                    ) : null}
                  </div>
