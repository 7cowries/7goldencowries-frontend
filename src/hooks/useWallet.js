...
import { fetchUserProfile } from '../utils/api'; // We will create this next
...
export const WalletAndUserProvider = ({ children }) => {
    ...
    useEffect(() => {
        // This function runs whenever the wallet status changes (connect/disconnect)
        const unsubscribe = tonConnectUI.onStatusChange(async (walletInfo) => {
            setWallet(walletInfo);
            if (walletInfo) {
                // Wallet connected, now fetch the user profile from your backend
                ...
                // Wallet disconnected, clear the user data
                setUser(null);
            }
        });
